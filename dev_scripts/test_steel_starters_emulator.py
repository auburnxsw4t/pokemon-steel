...
def run_choice(runner, choice):
    ...
    try:
        ...
        # Kyle finishes first and gives control back; registration itself is an
        # explicit A-button interaction at the clerk counter.
        emulator.advance(lambda: emulator.var(0xfa) == 2)
        assert emulator.flag(0x2d), (name, 'registration flag')
        emulator.goto(5, 5)
        emulator.walk(64, 8)
        emulator.tap(1)
        emulator.advance(lambda: emulator.var(0xfa) == 3)
        assert emulator.flag(0x39), (name, 'registration completion flag')
        assert emulator.item_quantity(874) == 1, (name, 'Trainer License missing')
